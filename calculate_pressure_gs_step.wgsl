...
@group(0) @binding(5) var<storage> divFieldIn : array<f32>;
@group(0) @binding(6) var<storage, read_write> tempFieldOut : array<f32>;
@group(0) @binding(7) var<storage> pressureFieldIn : array<f32>;
...
fn csMain(@builtin(global_invocation_id) cell : vec3<u32>) {
    ...
    tempFieldOut[idx] = (
        pressureFieldIn[cellIndex(vec2u(i+1, j))]
        + pressureFieldIn[cellIndex(vec2u(i-1, j))]
        + pressureFieldIn[cellIndex(vec2u(i, j+1))]
        + pressureFieldIn[cellIndex(vec2u(i, j-1))]
        - divFieldIn[idx]
    ) / 4.;
}

...
@group(0) @binding(5) var<storage> fieldXIn : array<f32>;
@group(0) @binding(6) var<storage, read_write> divFieldOut : array<f32>;
@group(0) @binding(7) var<storage> fieldYIn : array<f32>;
...
fn csMain(@builtin(global_invocation_id) cell : vec3<u32>) {
    ...
    divFieldOut[idx] = (
        fieldXIn[cellIndex(vec2u(i+1, j))]
        - fieldXIn[cellIndex(vec2u(i-1, j))]
        + fieldYIn[cellIndex(vec2u(i, j+1))]
        - fieldYIn[cellIndex(vec2u(i, j-1))]
    ) / 2. / gridSide;
}

...
@group(0) @binding(4) var<storage> setBoundsType : u32; // type of setBounds
...
fn csMain(@builtin(global_invocation_id) cell : vec3<u32>) {
    ...
    let verticalWalls : f32 = 1.;
    let horizontalWalls : f32 = 1.;
    ...
    let gridSize : u32 = u32(grid.x * grid.y);
    let gridWidth : u32 = u32(grid.x);
    let gridHeight : u32 = u32(grid.y);

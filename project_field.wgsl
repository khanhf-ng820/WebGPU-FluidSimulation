...
@group(0) @binding(5) var<storage> pressureFieldIn : array<f32>;
@group(0) @binding(6) var<storage, read_write> fieldXOut : array<f32>;
@group(0) @binding(8) var<storage, read_write> fieldYOut : array<f32>;
...
fn csMain(@builtin(global_invocation_id) cell : vec3<u32>) {
    ...
    let gridSide : f32 = grid.x; // Define the GRID_SIDE later
    let idx : u32 = cellIndex(cell.xy);
    let i : u32 = cell.x;
    let j : u32 = cell.y;

    fieldXOut[idx] -= (
        pressureFieldIn[cellIndex(vec2u(i+1, j))] - pressureFieldIn[cellIndex(vec2u(i-1, j))]
    ) / 2. * gridSide;
    fieldYOut[idx] -= (
        pressureFieldIn[cellIndex(vec2u(i, j+1))] - pressureFieldIn[cellIndex(vec2u(i, j-1))]
    ) / 2. * gridSide;
}
